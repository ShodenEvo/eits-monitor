...
go build -trimpath -ldflags '-s -w -H=windowsgui' -o build/windows/eits-agent-engine.exe ./cmd/eits-agent
$Manifest = Join-Path $Root 'assets\windows-common-controls.manifest'
go build -trimpath -ldflags '-s -w -H=windowsgui' -o build/windows/eits-agent-updater.exe ./cmd/eits-agent-updater
